...
export default function Home() {
	...
	useEffect(() => {
		const jsonIsSaveLogin = localStorage.getItem('isSaveLogin');
		const flagOfSaveLogin = JSON.parse(jsonIsSaveLogin);
		if (flagOfSaveLogin) {

			const jsonLoginInfo = localStorage.getItem('loginInfo');
			const savedLoginInfo = JSON.parse(jsonLoginInfo);
			...
			<div className={`${c.loginArea} ${c.stagger}`}>
				...
				<section className={c.loginOptionSection}>
					<div className={c.loginOptionItem}>
						<Checkbox onClick={() => setIsSaveLogin(!isSaveLogin)} />
