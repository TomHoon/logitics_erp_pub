...
const numberToKorean = (num) => Number(num || 0).toLocaleString();

export default function SalaryStatementModal({ open, setOpen, data }) {
	const totalPay = Number(data?.totalPayAmount || 0);
	const totalDeduction = Number(data?.totalDeductionAmount || 0);
	const realPay = Number(data?.realPayAmount || 0);

	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogContent
				showCloseButton={false}
				className="flex w-[560px] max-w-[560px] max-h-[90vh] flex-col p-0 overflow-hidden rounded-[14px] border-0"
			>
				{/* Header */}
				<div className="flex h-[58px] shrink-0 items-center justify-between bg-[#1F4478] px-5 text-white">
					...
					<div className="flex items-center gap-2">
						...
						<button
							...
							className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-[8px] bg-[#3569A5]"
						>
							<X size={20} />
						</button>
					</div>
				...
				<div className="flex items-center justify-between border-b bg-[#F8FAFC] px-5 py-4">
					...
					<div className="text-right text-[13px] text-[#9CA3AF]">
						<p>
							# PAY-{data?.year}-{String(data?.month).padStart(2, '0')}-
							{String(data?.payrollId ?? '-').padStart(4, '0')}
						</p>
						<p className="mt-2 flex items-center gap-1 justify-end">
							<CalendarDays size={14} /> {data?.paymentDate?.replaceAll('-', '.') || '-'}
						</p>
					</div>
				</div>
				...
				<div className="bg-white px-5 py-4">
					...
					<div className="grid grid-cols-4 border-t border-l text-[14px]">
						...
						<InfoValue>{data?.employeeNo || '-'}</InfoValue>

						<InfoLabel>부서</InfoLabel>
						<InfoValue>{data?.departmentName || '-'}</InfoValue>
						<InfoLabel>직급</InfoLabel>
						<InfoValue>{data?.positionName || '-'}</InfoValue>

						<InfoLabel>지급연월</InfoLabel>
						<InfoValue>
							{data?.year}.{String(data?.month).padStart(2, '0')} (
							{data?.paymentDate?.replaceAll('-', '.') || '-'} 지급)
						</InfoValue>
						<InfoLabel>근속연수</InfoLabel>
						<InfoValue>-</InfoValue>

						<InfoLabel>급여상태</InfoLabel>
						<InfoValue>{data?.payrollStatusText || '-'}</InfoValue>
					</div>
				</div>
				...
				<div className="grid grid-cols-2 border-y">
					<AmountBox
						title="지급항목"
						totalLabel="지급합계"
						total={numberToKorean(totalPay)}
						color="blue"
						icon={<TrendingUp size={15} />}
						rows={[
							['기본급', numberToKorean(data?.basicSalaryAmount)],
							['수당합계', numberToKorean(data?.totalAllowanceAmount)],
						]}
					/>
					...
					<AmountBox
						title="공제항목"
						totalLabel="공제합계"
						total={numberToKorean(totalDeduction)}
						color="red"
						icon={<TrendingDown size={15} />}
						rows={[
							['국민연금 (4.5%)', numberToKorean(data?.nationalPensionAmount)],
							['건강보험 (3.545%)', numberToKorean(data?.healthInsuranceAmount)],
							['고용보험 (0.9%)', numberToKorean(data?.employmentInsuranceAmount)],
							['소득세 (0.3%)', numberToKorean(data?.incomeTaxAmount)],
						]}
					/>
				</div>
					...
					<div className="flex items-center justify-between">
						<div>
							...
							<p className="mt-2 text-[13px] text-[#93C5FD]">
								{numberToKorean(totalPay)} - {numberToKorean(totalDeduction)}
							</p>
						</div>

						<div className="text-right">
							<p className="text-[34px] font-extrabold">{numberToKorean(realPay)}</p>
				...
				<div className="flex h-[42px] items-center justify-between border-b bg-[#EFF6FF] px-5 text-[13px] font-bold">
					...
					<div className="flex gap-3 text-[#4B5563]">
						<span className="text-[#2563EB]">● 출근 -일</span>
						<span>|</span>
						<span className="text-[#16A34A]">● 연차 -일</span>
						<span>|</span>
						<span className="text-[#8B5CF6]">● 야근 -시간</span>
						<span>|</span>
						<span className="text-[#EA580C]">● 지각 -회</span>
					</div>
				</div>
				...
				<div className="relative bg-white px-5 py-4">
					...
					<p className="mt-5 text-[14px] leading-7 text-[#4B5563]">
						...
						<br />
						<b>지급일: {data?.paymentDate?.replaceAll('-', '.') || '-'}</b>
						<br />
				...
				{/* Footer */}
				<div className="flex h-[58px] shrink-0 items-center justify-between border-t bg-[#F8FAFC] px-5">
					...
					<div className="flex gap-2">
						<button className="flex h-[38px] cursor-pointer items-center gap-2 rounded-[6px] border px-4 text-[14px] font-bold text-[#4B5563]">
							<Mail size={16} />
							이메일 발송
						</button>
						<button className="flex h-[38px] cursor-pointer items-center gap-2 rounded-[6px] bg-[#1F4478] px-5 text-[14px] font-bold text-white">
							<Printer size={16} />
							인쇄
						</button>
						<button
							onClick={() => setOpen(false)}
							className="flex h-[38px] cursor-pointer items-center gap-2 rounded-[6px] border px-4 text-[14px] font-bold text-[#4B5563]"
						>
							<X size={16} />
							닫기
						</button>
