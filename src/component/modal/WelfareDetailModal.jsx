...
export default function WelfareDetailModal({ open, setOpen }) {
	return (
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogContent className="max-w-[520px] p-0 overflow-hidden rounded-[10px]">
				...
				<div className="p-5 space-y-5 bg-white">
					...
					<div className="rounded-lg border bg-[#F8FAFC] p-4">
						<div className="flex justify-between text-[12px] text-gray-500 mb-3">
							...
							<span className="flex items-center gap-1">
								<Calendar size={13} /> 신청일: 2025.07.01
							</span>
						</div>
					...
					<Section title="경조 정보">
						<InfoRow label="경조구분">
							<span className="text-pink-500 font-bold">본인결혼</span>
					...
					<Section title="지급 정보">
						<InfoRow label="지급금액">
							<CreditCard size={14} />
							<b className="text-[#1E3F73] text-[16px]">500,000원</b>
							<span className="text-gray-400">(오십만원정)</span>
						</InfoRow>
						<InfoRow label="지급계좌">
							<CreditCard size={14} /> 국민은행 12****34 (이영희)
						</InfoRow>
					...
					<Section title="첨부 서류">
						<FileItem
							name="결혼확인서_이영희_20250701.pdf"
							type="PDF · 245 KB · 2025.07.01 업로드"
						/>
						<FileItem
							name="청첩장_스캔본.jpg"
							type="JPG · 1.2 MB · 2025.07.01 업로드"
							red
						/>
					</Section>
...
function FileItem({ name, type, red }) {
	return (
